...
ruleList    = []
ruleCounter = 1
P           = random.uniform(0,1)                # added probabbility
...
def checkingRule(inputRule, inputState):  
    ...
    if thisCompOp == '=':
		...
		if(inputRule.probability >= 0.5):   #### addition
            		return True
        else:
            return False
...
def doAction(inputCellDict, inputActionL):
    ...
    for actionItem in inputActionL:                      # inputActionList
        if(actionItem.isApplicable()):                   # if time is 1
            ruleList[(actionItem.ruleID)-1].makePassive() # make active param. false
            actionCellNT = actionItem.TcellName		  # Target cell name
            if(actionItem.Toperation == CONST_INC):       # if the command operation is 'increase'
                newRetCellDict[actionCellNT].increaseByValue(actionItem.Tproperty, actionItem.Tvalue)  # then increase the value.
                Tbool = True
            ...
            actionCellNS = actionItem.ScellName		# Source cell name
            if(actionItem.Soperation == CONST_INC):	# if the source command operation is 'increase'
...
def doTest():
    ...
    constructFirstState()
    makeRulesPassive()
    currentState = Gmain.node['state1']['data']             # what is it doing? Gmain.node[][] returns the current state (state1 with its data = myState(.......))
